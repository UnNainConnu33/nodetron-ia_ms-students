...
/**
 * call "MSB.semiCircle" '{ "robots_amount" : 4, "opponent_id" : 0, "radius" : 1}'
 * if opponent_id == 1, then the robot goes to the ball.
 */
export default class semiCircle extends Strategies {
  name = 'semiCircle';
  ...
  public static declaration: ActionSchema = {
    ...
    handler(ctx: Context<{robots_amount: number, opponent_id: number, radius: number}>): void {
        ...
        state.assign.register([ctx.params.robots_amount, ctx.params.opponent_id, ctx.params.radius], 
            new semiCircle(ctx.params.robots_amount, ctx.params.opponent_id, ctx.params.radius))
    },
  }
  ...
  // This method works perfectly but isn't the most effective one
    const shifts_3 = [[this.radius, 0], [this.radius/2, -this.radius/2], [this.radius/2, this.radius/2]]
    const shifts_4 = [[this.radius, -this.radius/3], [this.radius, this.radius/3], [this.radius/2, -this.radius], [this.radius/2, this.radius]]
    const shifts_5 = [[this.radius, 0], [this.radius/2, -this.radius/2], [this.radius/2, this.radius/2], [0, -this.radius], [0, this.radius]]
    const shifts_6 = [[this.radius, -this.radius/3], [this.radius, this.radius/3], [this.radius/2, -this.radius], [this.radius/2, this.radius], [0, -1.33*this.radius], [0, 1.33*this.radius]]
